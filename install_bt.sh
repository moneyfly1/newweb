...
reset_admin_password() {
    ...
    if [ ! -f cboard.db ]; then
        ...
        echo "请确保已完成安装并启动过服务"
        echo ""; read -rp "按回车键继续..."; return 1
    fi

    if ! command -v sqlite3 &>/dev/null; then
        warn "sqlite3 工具未安装，正在安装..."
        if command -v apt-get &>/dev/null; then
...
update_code() {
    ...
    if [[ "${rebuild:-y}" =~ ^[Yy]$ ]]; then
        ...
        info "安装前端依赖..."
        cd frontend && npm install --legacy-peer-deps --omit=optional || { err "依赖安装失败"; cd ..; return 1; }
