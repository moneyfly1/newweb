...
func SecurityHeaders() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// img-src/media-src 允许 blob: —— 工单附件（鉴权下载）经 blob URL 预览图片/视频
		c.Header("Content-Security-Policy", "default-src 'self'; script-src 'self' https://telegram.org; style-src 'self' 'unsafe-inline'; img-src 'self' https: data: blob:; media-src 'self' blob:; font-src 'self' data: http://at.alicdn.com https://at.alicdn.com; connect-src 'self'; frame-src https://telegram.org; object-src 'none'; base-uri 'self'; form-action 'self'")
