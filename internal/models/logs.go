...
// SystemLog 系统运行日志（调度任务、邮件、支付、通知、安全、订单等各业务模块）
type SystemLog struct {
	ID        uint      `gorm:"primaryKey" json:"id"`
	Level     string    `gorm:"type:varchar(10);index;not null" json:"level"`  // info / warn / error
	Module    string    `gorm:"type:varchar(50);index;not null" json:"module"` // 业务模块名，如 scheduler/email/payment/notify/security
...
// RetentionTable 描述一张可按保留期清理的日志表。
// 供定时任务（scheduler）与后台手动清理（admin_settings）共用同一份清单，
// 避免两处维护导致时间列不一致（如 login_history 用 login_time 而非 created_at）。
...
// RetentionLogTables 全部需要按 created_at/login_time 保留期清理的日志表。
var RetentionLogTables = []RetentionTable{
	{&AuditLog{}, "audit_logs", "created_at"},
	{&RegistrationLog{}, "registration_logs", "created_at"},
	{&SubscriptionLog{}, "subscription_logs", "created_at"},
	{&BalanceLog{}, "balance_logs", "created_at"},
	{&CommissionLog{}, "commission_logs", "created_at"},
	{&SystemLog{}, "system_logs", "created_at"},
	{&LoginHistory{}, "login_history", "login_time"},
	{&LoginAttempt{}, "login_attempts", "created_at"},
	{&UserActivity{}, "user_activities", "created_at"},
	{&VerificationAttempt{}, "verification_attempts", "created_at"},
}
